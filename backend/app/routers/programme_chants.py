...
def ajouter_chant_au_programme(
    programme_id: int, payload: ProgrammeChantCreate, db: Session = Depends(get_db)
):
    if not programmes_service.get_programme_by_id(db, programme_id):
        raise HTTPException(status_code=404, detail="Programme non trouvé")
    try:
...
def modifier_ordre_chant(
    ...
    updated = programme_chants_service.update_ordre(
        db, programme_id, chant_id, payload.ordre
    )
    if not updated:
...
def retirer_chant_du_programme(
    programme_id: int, chant_id: int, db: Session = Depends(get_db)
):
    removed = programme_chants_service.remove_chant_from_programme(
        db, programme_id, chant_id
    )
    if not removed:
        raise HTTPException(status_code=404, detail="Association non trouvée")